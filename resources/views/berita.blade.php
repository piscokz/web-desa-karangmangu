...
@section('title', 'Berita Desa – Desa Karangmangu')
            ...
            <div id="articlesGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($articles as $idx => $item)
                    <a href="{{ route('article.show', $item->id) }}"
                        ...
                        data-title="{{ Str::lower($item->title) }}" data-category="{{ $item->category }}">
                        <div class="relative overflow-hidden">
                            ...
                            <span
                                class="absolute top-4 right-4 bg-white bg-opacity-80 text-gray-800 px-3 py-1 rounded-full text-xs">
                                {{ \Carbon\Carbon::parse($item->date)->format('d M Y') }}
                            </span>
                        </div>
                        <div class="p-6 flex flex-col h-48 justify-between">
                            <h2 class="text-lg font-semibold text-gray-800 line-clamp-2">{{ Str::limit($item->title, 30) }}
                            </h2>
                            <span class="text-gray-600 text-sm mt-2 line-clamp-3">
                            ...
                            <div class="mt-4 flex justify-between items-center">
                                ...
                                <div class="flex items-center gap-2">
                                    <img src="{{ $item->author_photo ? asset('storage/' . $item->author_photo) : 'https://via.placeholder.com/40' }}"
                                        alt="{{ $item->author_name }}" class="w-8 h-8 rounded-full object-cover">
