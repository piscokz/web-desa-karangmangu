@php
    ...
    $raw = $kontak->no_telepon
        ? preg_replace('/\D/', '', $kontak->no_telepon)
        : '';
    // Jika mulai dengan 0, ganti ke country code 62
...
<footer id="footer" class="bg-black text-white py-10 mt-8 opacity-0 transition-opacity duration-1000">
  <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8 text-sm">
    ...
    <!-- Kolom 1: Logo & Alamat -->
    <div>
      <img src="{{ asset('images/Logo_Kabupaten_kuningan.png') }}"
    ...
    <div>
      ...
      <p class="text-gray-300 mb-1">
        <a href="tel:{{ str_replace([' ', '-'], '', $formattedPhone) }}"
      ...
      @if($kontak->email)
      <a href="{{ $emailLink }}"
         class="text-gray-300 hover:underline block">
        {{ $kontak->email }}
      </a>
      @endif
    </div>
    ...
    <div>
      ...
      <a href="{{ route('galeri') }}"
         class="text-gray-300 hover:underline block">Galeri Foto Desa</a>
      @guest
      <a href="{{ route('login') }}"
         class="text-gray-300 hover:underline block mt-2">Login</a>
      @endguest
      @auth
      <a href="{{ route('admin.dashboard') }}"
         class="text-gray-300 hover:underline block mt-2">Admin Page</a>
      @endauth
    </div>
  ...
  <div class="border-t border-green-500 mt-10 pt-4 text-center text-xs text-gray-400">
    Redesigned By &copy; <script>document.write(new Date().getFullYear());</script> Desa Karangmangu. All rights reserved.
    <br>
