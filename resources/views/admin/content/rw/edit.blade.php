...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-2xl font-bold text-green-800 mb-6">Edit RW</h1>
    ...
    <div class="bg-white shadow rounded-2xl p-6">
        <form action="{{ route('rw.update', $rw->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="nomor_rw" class="block text-gray-700 font-medium mb-1">Nomor RW</label>
                <input type="text" name="nomor_rw" id="nomor_rw" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-200" value="{{ $rw->nomor_rw }}" required placeholder="Masukkan Nomor RW">
            </div>
            <div class="form-group">
                ...
                <select name="id_dusun" id="id_dusun" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-200" required>
                    ...
                    @foreach ($dusuns as $dusun)
                        <option value="{{ $dusun->id }}" {{ $rw->id_dusun == $dusun->id ? 'selected' : '' }}>{{ $dusun->nama_dusun }}</option>
                    @endforeach
                </select>
            </div>
