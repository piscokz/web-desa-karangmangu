...
@section('content')
    <div class="container mx-auto px-4 py-8" x-data="{
        ...
        jenisOther: '{{ old(
            'jenis_kelamin_other',
            in_array(old('jenis_kelamin', $penduduk->jenis_kelamin), ['Laki-laki', 'Perempuan'])
                ? ''
                : old('jenis_kelamin', $penduduk->jenis_kelamin),
        ) }}',
        ...
        agamaOther: '{{ old(
            'agama_other',
            in_array(old('agama', $penduduk->agama), ['Islam', 'Kristen', 'Katolik', 'Hindu', 'Buddha', 'Konghucu'])
                ? ''
                : old('agama', $penduduk->agama),
        ) }}',
    
        status: '{{ in_array(old('status_perkawinan', $penduduk->status_perkawinan), [
            'Belum Kawin',
            'Kawin',
            'Cerai Hidup',
            'Cerai Mati',
        ...
        statusOther: '{{ old(
            ...
            in_array(old('status_perkawinan', $penduduk->status_perkawinan), [
                'Belum Kawin',
                'Kawin',
                'Cerai Hidup',
                'Cerai Mati',
        ...
        golOther: '{{ old(
            'gol_darah_other',
            in_array(old('gol_darah', $penduduk->gol_darah), ['A', 'B', 'AB', 'O'])
                ? ''
                : old('gol_darah', $penduduk->gol_darah),
        ) }}',
        ...
        shdkOther: '{{ old(
            'shdk_other',
            in_array(old('shdk', $penduduk->shdk), ['Kepala Keluarga', 'Istri', 'Anak', 'Orang Tua'])
                ? ''
                : old('shdk', $penduduk->shdk),
        ) }}'
    }">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <template x-if="jenis==='Lainnya'">
                            <input type="text" name="jenis_kelamin_other" placeholder="Masukkan manual..."
                                x-model="jenisOther" :value="jenisOther"
                    ...
                    <div>
                        ...
                        <template x-if="agama==='Lainnya'">
                            <input type="text" name="agama_other" placeholder="Masukkan manual..." x-model="agamaOther"
                                :value="agamaOther"
                                class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-200" />
                        </template>
                    </div>
                    ...
                    <div>
                        ...
                        <template x-if="status==='Lainnya'">
                            <input type="text" name="status_perkawinan_other" placeholder="Masukkan manual..."
                                x-model="statusOther" :value="statusOther"
                                class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-200" />
                        </template>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="pendidikan" id="pendidikan"
                            ...
                            <option value="TK/PAUD"
                                {{ old('pendidikan', $penduduk->pendidikan) == 'TK/PAUD' ? 'selected' : '' }}>TK/PAUD
                            </option>
                            <option value="SD"
                            ...
                            <option value="DIPLOMA"
                                {{ old('pendidikan', $penduduk->pendidikan) == 'DIPLOMA' ? 'selected' : '' }}>DIPLOMA
                            </option>
                            <option value="SARJANA (S1)"
                                {{ old('pendidikan', $penduduk->pendidikan) == 'SARJANA (S1)' ? 'selected' : '' }}>SARJANA
                                (S1)</option>
                            <option value="MAJISTER (S2)"
                                {{ old('pendidikan', $penduduk->pendidikan) == 'MAJISTER (S2)' ? 'selected' : '' }}>
                                MAJISTER (S2)</option>
                            <option value="DOKTOR (S3)"
                                {{ old('pendidikan', $penduduk->pendidikan) == 'DOKTOR (S3)' ? 'selected' : '' }}>DOKTOR
                                (S3)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <template x-if="gol==='Lainnya'">
                            <input type="text" name="gol_darah_other" placeholder="Masukkan manual..."
                                x-model="golOther" :value="golOther"
                                class="mt-2 w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-200" />
                        </template>
                    </div>
                    ...
                    {{-- Kartu Keluarga (Select2 di dalam div yang sama) --}}
                    <div>
                        {{-- CDN Select2 CSS --}}
                    ...
                    <div>
                        ...
                        <select name="disabilitas"
                            ...
                            @foreach (['Tidak Cacat', 'Tuna Daksa/Cacat Tubuh', 'Tuna Netra/Buta', 'Tuna Rungu', 'Tuna Wicara', 'Tuna Rungu & Wicara', 'Tuna Netra & Cacat Tubuh', 'Tuna Netra, Rungu & Wicara', 'Tuna Rungu, Wicara & Cacat Tubuh', 'Tuna Rungu, Wicara, Netra & Cacat Tubuh', 'Cacat Mental/Retardasi', 'Mantan Penderita Gangguan Jiwa', 'Cacat Fisik & Mental', 'Tuna Grahita'] as $d)
                                <option value="{{ $d }}"
