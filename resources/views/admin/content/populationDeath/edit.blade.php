    ...
    <form action="{{ route('kematian.update', $populationDeath->id) }}" method="POST"
        ...
        <div>
            ...
            <select id="penduduk_id" name="penduduk_id"
                ...
                @foreach($residents as $r)
                    <option value="{{ $r->id }}"
                        {{ $r->id == old('penduduk_id', $populationDeath->penduduk_id) ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input id="tanggal_meninggal" type="date" name="tanggal_meninggal"
                   value="{{ old('tanggal_meninggal', $populationDeath->tanggal_meninggal) }}"
                   class="w-full bg-white border border-gray-300 text-gray-700 rounded px-3 py-2 focus:ring-2 focus:ring-blue-500"
                   required>
        ...
        <div>
            ...
            <input id="penyebab" type="text" name="penyebab"
                   value="{{ old('penyebab', $populationDeath->penyebab) }}"
        ...
        <div>
            ...
            <textarea id="keterangan" name="keterangan" rows="3"
                      class="w-full bg-white border border-gray-300 text-gray-700 rounded px-3 py-2 focus:ring-2 focus:ring-blue-500"
                      placeholder="Tambahan keterangan...">{{ old('keterangan', $populationDeath->keterangan) }}</textarea>
            @error('keterangan')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="col-span-2 flex justify-end space-x-3 pt-4">
            <button type="submit"
                    class="bg-yellow-500 hover:bg-yellow-600 text-gray-900 px-5 py-2 rounded shadow transition">
                💾 Update
            </button>
            <a href="{{ route('kematian.index') }}"
